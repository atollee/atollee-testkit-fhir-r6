...
export function runFhirVersionParameterTests(context: ITestContext) {
    ...
    it("FHIR Version Parameter - JSON with current version", async () => {
        ...
        assertEquals(response.headers.get("Content-Type")?.includes("fhirVersion=5.0"), true, "Response should specify FHIR version 5.0");
    ...
    it("FHIR Version Parameter - XML with current version", async () => {
        ...
        if (context.isXmlSupported()) {
            ...
            assertEquals(response.success, false, "Request should be not successful as XML is not supported");
        }
    });
    ...
    it("FHIR Version Parameter - Unsupported version", async () => {
        ...
        assertEquals(response.status, 406, "Should return 406 Not Acceptable for unsupported version");
    });
    ...
    it("FHIR Version Parameter - Create with version specified", async () => {
        ...
        assertEquals(response.success, true, "Create request should be successful");
        assertEquals(response.headers.get("Content-Type")?.includes("fhirVersion=5.0"), true, "Response should specify FHIR version 5.0");
    });
    ...
    it("FHIR Version Parameter - Mismatched versions", async () => {
        ...
        assertEquals(response.success, false, "Request with mismatched versions should fail");
        assertEquals(response.status, 400, "Should return 400 Bad Request for mismatched versions");
    });
