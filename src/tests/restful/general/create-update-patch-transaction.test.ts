...
export function runCreateUpdatePatchTransactionTests(_context: ITestContext) {
    ...
    it("Create - Prefer return=representation", async () => {
        ...
        assertExists(
            ...
            "Returned resource should have an ID",
        );
    });
    ...
    it("Patch - Prefer return=representation", async () => {
        ...
        assertExists(
            patchResponse.jsonBody,
            "Response should include the updated resource",
        );
        const responsePatient = patchResponse.jsonBody as Patient;
        assertEquals(
            responsePatient.active,
            true,
            "Patient should be updated with the patched value",
        );
    });
    ...
    it("Transaction - Prefer return=minimal", async () => {
        ...
        assertEquals(
            response.jsonBody.resourceType,
            "Bundle",
            "Returned resource should be a Bundle",
        );
        assertEquals(
            bundle.type,
            "transaction-response",
            "Bundle should be a transaction-response",
        );
        ...
        assertEquals(
            ...
            "201 Created",
            "Entry response should indicate successful creation",
        );
        assertExists(
            entry.response.location,
            "Entry response should include a location",
        );
