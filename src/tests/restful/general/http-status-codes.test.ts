...
export function runHttpStatusCodeTests(context: ITestContext) {
    ...
    it("HTTP Status Codes - 200 OK for successful read", async () => {
        ...
        assertEquals(
            response.success,
            true,
            "Successful read should return true success",
        );
        assertEquals(
            response.status,
            200,
            "Successful read should return 200 OK",
        );
    });
    ...
    it("HTTP Status Codes - 201 Created for successful create", async () => {
        ...
        assertEquals(
            response.success,
            true,
            "Successful create should return true success",
        );
        assertEquals(
            response.status,
            201,
            "Successful create should return 201 Created",
        );
        assertExists(
            response.headers.get("Location"),
            "Created resource should have a Location header",
        );
    });
    ...
    it("HTTP Status Codes - 404 Not Found for non-existent resource", async () => {
        ...
        assertEquals(
            response.success,
            false,
            "Request for non-existent resource should fail",
        );
        assertEquals(
            response.status,
            404,
    ...
    it("HTTP Status Codes - 422 Unprocessable Entity for valid format but invalid content", async () => {
        ...
        assertEquals(
            response.status,
            422,
            "Invalid content should return 422 Unprocessable Entity",
        );
    });
    ...
    it("HTTP Status Codes - OperationOutcome for 4xx errors", async () => {
        ...
        assertEquals(
            ...
            "OperationOutcome",
            "4xx error should return an OperationOutcome",
        );
        assertExists(
            operationOutcome.issue,
            "OperationOutcome should have an issue array",
        );
        assertExists(
            operationOutcome.issue[0].severity,
            "Issue should have a severity",
        );
        assertExists(
            operationOutcome.issue[0].code,
            "Issue should have a code",
        );

        assertEquals(
            operationOutcome.issue[0].code,
            "not-found",
            "Issue code should be 'not-found' for a non-existent resource",
        );
    });
