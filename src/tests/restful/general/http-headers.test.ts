...
export function runHttpHeaderTests(context: ITestContext) {
    ...
    it("HTTP Headers - If-Match for conditional update", async () => {
        ...
        assertEquals(updateResponse.success, true, "Conditional update should be successful");
        assertEquals(updateResponse.status, 200, "Successful update should return 200 OK");
    });
    ...
    it("HTTP Headers - If-Modified-Since for conditional read", async () => {
        ...
        assertEquals(response.status === 200 || response.status === 304, true, "Should return 200 OK if modified, or 304 Not Modified");
    });
    ...
    it("HTTP Headers - If-None-Exist for conditional create", async () => {
        ...
        const response = await fetchWrapper({
            ...
            method: "POST",
            headers: {
                "If-None-Exist": "identifier=http://example.com/identifiers|12345"
            },
            body: JSON.stringify(newPatient)
        });

        assertEquals(response.status === 201 || response.status === 200, true, "Should return 201 Created if new, or 200 OK if existing");
    });
    ...
    it("HTTP Headers - If-None-Match for conditional read", async () => {
        ...
        assertEquals(response.status, 304, "Should return 304 Not Modified if ETag matches");
    });
    ...
    it("HTTP Headers - Last-Modified in response", async () => {
        ...
        assertExists(lastModified, "Response should include a Last-Modified header");
        assertEquals(isNaN(Date.parse(lastModified!)), false, "Last-Modified should be a valid date");
    });
    ...
    it("HTTP Headers - Location in create response", async () => {
        ...
        assertEquals(location?.startsWith(context.getBaseUrl()), true, "Location should be a full URL");
    });
    ...
    it("HTTP Headers - Content-Location in async response", async () => {
        ...
        assertEquals(response.status, 202, "Should return 202 Accepted for async operations");
        const contentLocation = response.headers.get("Content-Location");
        assertExists(contentLocation, "Response should include a Content-Location header");
        assertEquals(contentLocation?.startsWith(context.getBaseUrl()), true, "Content-Location should be a full URL");
    });
}
