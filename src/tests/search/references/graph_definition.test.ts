...
export function runGraphDefinitionTests(context: ITestContext) {
    let graphDefinition: GraphDefinition;

    it("Should create a GraphDefinition for testing", async () => {
        graphDefinition = await createTestGraphDefinition(context, {
    ...
    it("Should include resources based on GraphDefinition when using _graph", async () => {
        ...
        assertExists(includedCondition, "Bundle should include the Condition");
    });
    ...
    it("Should handle _graph parameter with non-existent GraphDefinition", async () => {
        ...
        const response = await fetchWrapper({
            authorized: true,
            relativeUrl: `Patient?_id=${patient.id}&_graph=non-existent-graph`,
        });
    ...
    it("Should handle _graph parameter with invalid GraphDefinition reference", async () => {
        ...
        const response = await fetchWrapper({
            authorized: true,
            relativeUrl: `Patient?_id=${patient.id}&_graph=InvalidReference`,
        });
