...
export function runModifiersTests(context: ITestContext) {
    it("Should support modifiers on search parameters", async () => {
        ...
        await createTestPatient(context, { nameUndefined: true }); // Patient without a name
        ...
        assertEquals(
            response.status,
            200,
            "Server should process search with modifier successfully",
        );
    ...
    if (context.isHapiBugsDisallowed()) {
        it("Should reject search with unsupported modifier", async () => {
            ...
            assertEquals(
                response.status,
                400,
                "Server should reject search with unsupported modifier",
            );
    ...
    if (context.enforceMultipleModifiersDisallowed()) {
        it("Should allow only a single modifier per search parameter", async () => {
            ...
            assertEquals(
                response.status,
                400,
                "Server should reject search with multiple modifiers",
            );
            const operationOutcome = response.jsonBody as OperationOutcome;
            assertExists(
                operationOutcome.issue,
    ...
    it("Should apply modifier to all values in OR-joined search", async () => {
        ...
        await createTestPatient(context, {
            name: [{ given: [uniqueNameA.toLowerCase()] }],
        });
        await createTestPatient(context, {
            name: [{ given: [uniqueNameB.toLowerCase()] }],
        });
        ...
        assertEquals(
            response.status,
            200,
            "Server should process search with modifier on OR-joined values successfully",
        );
        const bundle = response.jsonBody as Bundle;
        assertExists(bundle.entry, "Bundle should contain entries");
        assertEquals(
        ...
        bundle.entry.forEach((entry) => {
            ...
            assertTrue(
                patient.name?.[0].given?.includes(uniqueNameA) ||
                    patient.name?.[0].given?.includes(uniqueNameB),
