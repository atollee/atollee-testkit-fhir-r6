...
export function runAboveModifierReferenceTests(context: ITestContext) {
    if (context.isLocationAboveModifierSupported()) {
        it("Should find procedures in a specific location and its parent locations", async () => {
            ...
            const procedureInFloor = await createTestProcedure(context, {
            ...
            assertEquals(
                response.status,
                200,
                "Server should process search with above modifier successfully",
            );
            ...
            assertTrue(
                procedureIds.includes(procedureInRoom.id),
                "Results should include procedure in the room",
            );
            assertTrue(
                procedureIds.includes(procedureInFloor.id),
                "Results should include procedure in the floor",
            );
            assertTrue(
        ...
        it("Should handle different reference formats with above modifier", async () => {
            ...
            for (const format of formats) {
                ...
                assertEquals(
                    response.status,
                    200,
                    `Server should process search with above modifier and reference format ${format} successfully`,
                );
                const bundle = response.jsonBody as Bundle;
                assertExists(bundle.entry, "Bundle should contain entries");
                assertEquals(
