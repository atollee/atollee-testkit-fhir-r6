...
export function runBelowModifierReferenceHierarchicalTests(context: ITestContext) {
    it("Should find procedures in a specific location and its child locations", async () => {
        ...
        const procedureInFloor1 = await createTestProcedure(context, { location: { reference: `Location/${floorA1.id}` } });
        const procedureInRoom101 = await createTestProcedure(context, { location: { reference: `Location/${roomA101.id}` } });
        const procedureInFloor2 = await createTestProcedure(context, { location: { reference: `Location/${floorA2.id}` } });
        const procedureInRoom201 = await createTestProcedure(context, { location: { reference: `Location/${roomA201.id}` } });
        ...
        assertEquals(response.status, 200, "Server should process search with below modifier successfully");
        const bundle = response.jsonBody as Bundle;
        assertExists(bundle.entry, "Bundle should contain entries");
        assertEquals(bundle.entry.length, 5, "Bundle should contain five matching Procedures");
        ...
        assertTrue(procedureIds.includes(procedureInFloor1.id), "Results should include procedure on floor 1");
        assertTrue(procedureIds.includes(procedureInRoom101.id), "Results should include procedure in room 101");
        assertTrue(procedureIds.includes(procedureInFloor2.id), "Results should include procedure on floor 2");
        assertTrue(procedureIds.includes(procedureInRoom201.id), "Results should include procedure in room 201");
    ...
    it("Should handle different reference formats with below modifier", async () => {
        ...
        for (const format of formats) {
            ...
            assertEquals(response.status, 200, `Server should process search with below modifier and reference format ${format} successfully`);
            const bundle = response.jsonBody as Bundle;
            assertExists(bundle.entry, "Bundle should contain entries");
    ...
    it("Should not apply below modifier to non-hierarchical references", async () => {
        ...
        const linkedPatient = await createTestPatient(context, { 
            link: [{ other: { reference: `Patient/${patient.id}` }, type: "seealso" }] 
        });
