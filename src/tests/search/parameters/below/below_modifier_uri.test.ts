...
export function runBelowModifierUriTests(context: ITestContext) {
    it("Should find ValueSets with URLs below the specified URL", async () => {
        ...
        await createTestValueSet(context, {
            url: uniqueString("http://example.com/fhir/ValueSet/456"),
            name: "TestValueSet6",
            status: "active",
        });
        ...
        assertEquals(
            response.status,
            200,
            "Server should process search with below modifier on URI successfully",
        );
        ...
        assertEquals(
            bundle.entry.length,
            5,
            "Bundle should contain five matching ValueSets",
        );
        ...
        assertTrue(
            urls.includes(`${baseUrl}/ValueSet`),
            "Results should include child URL",
        );
        assertTrue(
            urls.includes(`${baseUrl}/ValueSet/123`),
            "Results should include descendant URL",
        );
        assertTrue(
            urls.includes(`${baseUrl}/ValueSet/123/_history`),
            "Results should include descendant URL",
        );
        assertTrue(
            urls.includes(`${baseUrl}/ValueSet/123/_history/1`),
            "Results should include descendant URL",
        );
        assertFalse(
    ...
    if (context.isHapiBugsDisallowed()) {
        it("Should not match URNs when using below modifier", async () => {
            ...
            const urn =
                `urn:oid:2.16.840.${uniqueNumberValue}.113883.4.642.3.508`;
            await createTestValueSet(context, {
                url: urn,
            ...
            assertEquals(
                response.status,
                200,
                "Server should process search with below modifier on URN",
            );
            const bundle = response.jsonBody as Bundle;
            assertEquals(
                bundle.entry?.length ?? 0,
                0,
                "Bundle should not contain any matching ValueSets",
            );
        });
    }
    ...
    it("Should handle partial URL matching", async () => {
        ...
        assertEquals(
            response.status,
            200,
            "Server should process search with below modifier on partial URL",
        );
        const bundle = response.jsonBody as Bundle;
        assertExists(bundle.entry, "Bundle should contain entries");
        assertTrue(
            bundle.entry.length > 0,
            "Bundle should contain at least one matching ValueSet",
        );
        const valueSet = bundle.entry[0].resource as ValueSet;
        assertEquals(
            valueSet.url,
            `${baseUrl}/ValueSet/partial`,
            "Returned ValueSet should have the correct URL",
        );
    });
    ...
    it("Should not match URLs that are not below the specified URL", async () => {
        ...
        await createTestValueSet(context, {
            url: uniqueString("http://acme.org/other/ValueSet/456"),
        ...
        assertEquals(
            bundle.entry.length,
            1,
            "Bundle should contain one matching ValueSet",
        );
        const valueSet = bundle.entry[0].resource as ValueSet;
        assertEquals(
            valueSet.url,
            `${baseUrl}/ValueSet/123`,
            "Returned ValueSet should have the correct URL",
        );
    });
}
