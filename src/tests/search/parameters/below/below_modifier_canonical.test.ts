...
export function runBelowModifierCanonicalTests(context: ITestContext) {
    if (
        context.isCanonicalUrlBelowModifierSupported() &&
        ...
        it("Should find resources with versions below the specified version", async () => {
            ...
            assertEquals(
                response.status,
                200,
                "Server should process search with below modifier on canonical reference successfully",
            );
            ...
            assertTrue(
                versions.includes("1.0.0"),
                "Results should include version 1.0.0",
            );
            assertTrue(
        ...
        it("Should handle searches without version specified", async () => {
            const baseUrl = uniqueString(
                "http://example.com/StructureDefinition/NoVersionSpecified",
            );
            ...
            assertEquals(
                response.status,
                200,
                "Server should process search without version specified",
            );
            const bundle = response.jsonBody as Bundle;
            assertExists(bundle.entry, "Bundle should contain entries");
            assertEquals(
